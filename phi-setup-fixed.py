...
def download_model(token=None):
    ...
    try:
        ...
        processor_kwargs = {
            ...
            "audio_compression_rate": 1.0,  # No compression (adjust as needed)
            "audio_downsample_rate": 16000,  # Common sample rate, e.g., 16kHz
            "audio_feat_stride": 10         # Stride in milliseconds or frames
        }
        if token:
...
def test_model(token=None):
    ...
    try:
        ...
        processor_kwargs = {
            "trust_remote_code": True,
            "audio_compression_rate": 1.0,
            "audio_downsample_rate": 16000,
            "audio_feat_stride": 10
        }
        if token:
            processor_kwargs["token"] = token
